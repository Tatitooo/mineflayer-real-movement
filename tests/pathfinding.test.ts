...
describe('A* Pathfinder', () => {
  ...
  it('avoids a tree obstacle', async () => {
    const world = new MockWorld()
  ...
  it('climbs a 1-block hill', async () => {
    const world = new MockWorld()
    for (let x = -2; x < 3; x++) {
      for (let z = -2; z <= 2; z++) {
  ...
  it('descends a 1-block hill', async () => {
    const world = new MockWorld()
    for (let x = -2; x < 3; x++) {
      for (let z = -2; z <= 2; z++) {
        world.setBlock(x, 1, z, { name: 'stone', stateId: 1, boundingBox: 'block', shapes: [[0, 0, 0, 1, 1, 1]] })
    ...
    const result = await pf.findPath(new Vec3(0, 2, 0), goal)
    expect(result.status).toBe('success')
    const last = result.path[result.path.length - 1]
    expect(last.pos.x).toBeCloseTo(5, 0)
    expect(last.pos.y).toBeCloseTo(1, 0)
  })
  it('returns noPath when completely blocked', async () => {
    const world = new MockWorld()
    ...
    const pf = createPathfinder(world)
    const goal = new GoalBlock(new Vec3(5, 1, 0))
    const result = await pf.findPath(new Vec3(0, 1, 0), goal, 2000)
    expect(result.status).toBe('noPath')
  })

  it('crosses a 1-block gap', async () => {
    const world = new MockWorld()
    for (let z = -1; z <= 1; z++) {
  ...
  it('respects maxSearchDistance in open world', async () => {
    const world = new MockWorld()
    createFlatGround(world, 0, 5)
    const pf = createPathfinder(world)
    const goal = new GoalBlock(new Vec3(4, 1, 0))
    // With maxSearchDistance = 2, the pathfinder should not find a path to x=4
    const result = await pf.findPath(new Vec3(0, 1, 0), goal, 10000, 2)
    expect(result.status).toBe('noPath')

    // Without maxSearchDistance, it should succeed
    const result2 = await pf.findPath(new Vec3(0, 1, 0), goal, 10000)
    expect(result2.status).toBe('success')
  })

  it('uses diagonal edges when corner is clear', async () => {
    const world = new MockWorld()
...
describe('Goals', () => {
  it('GoalBlock accepts positions within radius', async () => {
  ...
  it('GoalNear accepts positions within radius', async () => {
